...
if st.button("🩺 Check API Health"):
    render_response("Health Check", "/health")
...
# ── Graph ──────────────────────────────────────────────────
with tab5:
    graph_days = st.slider("📊 Graph Days", 1, 90, 30)

    if st.button("Get Graph Data", use_container_width=True):
        render_response(
            "📊 Graph Response",
            f"/graph/{coin_id}?days={graph_days}"
        )

# ── Sentiment ──────────────────────────────────────────────
with tab6:
    if st.button("Get Sentiment", use_container_width=True):
        render_response(
            "📢 Sentiment Response",
            f"/sentiment/{coin_id}"
        )
